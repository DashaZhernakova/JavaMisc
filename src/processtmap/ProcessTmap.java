...
public class ProcessTmap {
        ...
        for (File ch : dir.listFiles()) {
            if (ch.isDirectory())
                for (File child : ch.listFiles()){
                   if (child.getName().endsWith("cufflinks_out")){
                       File f = new File(child.getPath() +"/cuffcmp.transcripts.gtf.tmap");
                       if (f.exists())
    ...
    public void printTable(String[][] table) throws IOException{
        BufferedWriter out = new BufferedWriter (new FileWriter("/target/gpfs2/gcc/home/dasha/GeneticalGenomicsDatasets/expression_table.txt"));
        //BufferedWriter out = new BufferedWriter (new FileWriter("/Users/dashazhernakova/Documents/UMCG/GeneticalGenomicsDatasets/expression_table.txt"));
	...
	//ArrayList<ArrayList> table = new ArrayList();
        ...
        for (String f_name : fNames){
            ...
            while ((line = in.readLine()) != null){
               ...
               if ((code.equals("c")) || (code.equals("=")) || (code.equals("j"))){
                    ...
                    if (index >= 0){
                        ...
                        index = index + 1;//to leave the first row in the table for sample ids
                         ...
                         if (!cur_count.equals("0.0"))
                            table[index][cur_id] = Float.toString(Float.valueOf(cur_count.trim()).floatValue() + read_count);      
                         //if a new transcript 
                         else{
                            ...
                            table[index][cur_id] = Float.toString(read_count); //write RPKM
                         }
                    }
                }
            }
